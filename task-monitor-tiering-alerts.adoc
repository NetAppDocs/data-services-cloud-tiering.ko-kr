...
이 알림은 시스템 효율성을 개선하고 저장 비용을 줄이기 위한 "권장 사항"입니다. 이는 클러스터가 콜드 데이터의 20% 미만을 계층화하고 있음을 나타냅니다. 여기에는 데이터를 계층화하지 않는 클러스터도 포함됩니다. 링크를 제공합니다 https://www.netapp.com/data-services/tiering/tco-calculator/["클라우드 티어링 총 소유 비용 및 절감 계산기"^] 비용 절감액을 계산하는 데 도움이 됩니다.
